...
For architecture and product-direction work, treat `.env`, JSON/YAML config, shell env, copied snippets, and prompt transcripts as agent-readable material, not secret boundaries.
The supported agent path is brokered capability or session access through `vault`; `vault run` env injection is a user-only compatibility path and should not be described as the recommended agent model.
