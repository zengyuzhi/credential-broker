...
- Phase 1.1 grant-selection filters live in `vault_policy::session`: `select_attachable_grants(grants, agent_name)` returns `GrantSelection { attachable, skipped_confirmation }` — attaching any grant to a session MUST flow through this helper, never raw `list_grants_for_bundle()`. Three filters are enforced there: active (enabled + not expired), agent scope (`grant.agent_name == agent` or `"*"`), and `!require_confirmation`. Cross-agent privilege leaks are impossible only because this filter runs at issuance.
- `clamp_session_ttl(user_ttl: NonZeroU32, &[&Grant]) -> (NonZeroU32, Option<u32>)` caps the session TTL at the tightest positive `grant.ttl_minutes` across attached grants. CLI `session issue` must apply this before `issue_session`, otherwise a 10080-minute session can outlive a 60-minute grant cap.
- Proxy multi-grant disambiguation: `list_session_proxy_candidates` (renamed from `resolve_session_proxy_grant`) returns `Vec<SessionGrantCandidate>`. When len > 1 the caller passes `X-Vault-Connector: <name>` and/or `X-Vault-Capability: <name>`; the proxy returns `409 Conflict` on ambiguity, `403 Forbidden` on no match. Single-candidate case keeps backward-compat with provider-only routing.
- `Grant.max_requests` enforcement uses `session_grants.request_count` (migration 0006), NOT `sessions.request_count` (which is session-wide observability). `increment_session_grant_request_count(session_id, grant_id)` is fire-and-forget and only called on `2xx` upstream responses — a failed upstream call must not burn the caller's quota. Quota exhaustion returns `429 Too Many Requests`.
- `AttributionCtx.grant_id` is `Some(_)` only on the session branch — lease branch leaves it `None`, and the post-dispatch `session_grants` counter bump is gated on both `session_id` and `grant_id` being `Some`.
